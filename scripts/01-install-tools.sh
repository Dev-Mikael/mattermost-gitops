...
# Installs: kubectl, helm, flux CLI, kubeseal, kind (optional)
set -euo pipefail
...
install_on_linux() {
  ...
  if ! command -v kubectl &>/dev/null; then
    ...
    log_ok "kubectl ${KUBE_VER} installed"
  else
    log_ok "kubectl already installed: $(kubectl version --client --short 2>/dev/null || kubectl version --client)"
  fi
  ...
  if ! command -v flux &>/dev/null; then
    ...
    log_ok "Flux CLI installed: $(flux version --client)"
  else
    log_ok "Flux CLI already installed: $(flux version --client)"
  fi

  # ── kubeseal ─────────────────────────────────────────────
  if ! command -v kubeseal &>/dev/null; then
    log_step "Installing kubeseal (Sealed Secrets CLI)"
    KUBESEAL_VER=$(curl -sL https://api.github.com/repos/bitnami-labs/sealed-secrets/releases/latest \
      | grep '"tag_name"' | sed 's/.*"v\([^"]*\)".*/\1/')
    curl -sLo /tmp/kubeseal \
    ...
    log_ok "kubeseal ${KUBESEAL_VER} installed"
  else
    log_ok "kubeseal already installed: $(kubeseal --version)"
  fi

  # ── kind (optional — only needed for local dev) ──────────
  if ! command -v kind &>/dev/null; then
    log_step "Installing kind (for local dev clusters)"
    curl -sLo /tmp/kind \
      "https://kind.sigs.k8s.io/dl/latest/kind-linux-${ARCH_ALT}"
    chmod +x /tmp/kind
    sudo mv /tmp/kind /usr/local/bin/kind
    log_ok "kind installed: $(kind version)"
  else
    log_ok "kind already installed: $(kind version)"
  fi
}
...
install_on_mac() {
  ...
  log_ok "All tools installed via Homebrew"
}
